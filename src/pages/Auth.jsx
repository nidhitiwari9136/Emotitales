...
function Auth() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend agar success message bhej raha hai
      if (res.data.message || res.status === 200 || res.status === 201) {
        if (isLogin) {
          localStorage.setItem("user", username);
          // Force refresh ya navigate
          navigate("/");
          window.location.reload(); // Kabhi kabhi state update ke liye zaroori hota hai
        } else {
          setMessage("Registered successfully! Please login now.");
          setIsLogin(true); // Sign-in screen par le jayega
      ...
      // Backend se error message nikalne ke liye
      const errorMsg = err.response?.data?.error || err.response?.data?.message || "Server error. Check backend.";
      setMessage(errorMsg);
    }
  };
